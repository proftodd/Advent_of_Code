...
def test_substitute_fractional():
    r1 = Reaction(
        {
            'KHKGT': 5, 'QDVJ': 1, 'DCFZ': 154, 'GPVTF': 9,
            'HKGWZ': 48, 'NZVS': 29, 'PSHF': 154
        },
        ('FUEL', 1)
    )
    r2 = Reaction(
        {
            'GPVTF': 1, 'HKGWZ': 12, 'PSHF': 8
        },
        ('QDVJ', 9)
    )
    r = r1.substitute_fractional(r2)
    assert r.rcts['KHKGT'] == 5
    assert r.rcts['DCFZ'] == 154
    assert r.rcts['GPVTF'] == Fraction(82, 9)
    assert r.rcts['HKGWZ'] == Fraction(148, 3)
    assert r.rcts['NZVS'] == 29
    assert r.rcts['PSHF'] == Fraction(1394, 9)
